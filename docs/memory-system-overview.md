...
- ChatApp 主回复在加载范围后及最终请求组装后检查是否仍有有效 user/assistant 消息。自适应范围只剩系统消息时，从 DB 最近 500 条中自动找回最后一轮尚未获回复的用户输入，仅为本次请求绕过归档水位（仍遵守用户断点、角色隔离和私聊来源限制）；不修改全自动设置、水位或记忆，不恢复已获回复的旧轮次。确无输入时只提示再发一条消息，不要求用户维护上下文设置。本地、Instant Push 和即时对话上云共用此保护，避免 system-only 请求触发上游 `contents is not specified`。实现见 `utils/chatContextRecovery.ts`、`utils/chatRequestGuard.ts`。
